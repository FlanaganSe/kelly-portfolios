    ...
    <div class="hero-gradient min-h-screen">
      <div class="container mx-auto px-6 py-20">
        <div class="text-center mb-20">
          <h1 class="text-5xl md:text-7xl font-bold gradient-text mb-8 text-balance">Investment Articles</h1>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
          <For each={articles}>
            {(article) => (
              <article class="card card-hover p-8 group cursor-pointer">
                <div class="flex items-center mb-6">
                  <div
                    class={`w-16 h-16 bg-gradient-to-br ${article.gradient} rounded-xl flex items-center justify-center mr-4 shadow-lg`}
                  >
                    <Icon name="book" size={8} class="text-white" aria-label="Article icon" />
                  </div>
                  <div>
                    <div class="text-sm font-semibold text-slate-500">{article.category}</div>
                    <div class="text-sm text-slate-400">{article.readTime}</div>
                  </div>
                </div>
                <h2 class="text-2xl font-bold text-slate-900 mb-4 group-hover:text-blue-600 transition-colors text-balance">
                  {article.title}
                </h2>
                <p class="text-lg text-slate-600 leading-relaxed mb-6">{article.excerpt}</p>
                <div class="flex items-center text-blue-600 group-hover:text-blue-700 transition-all">
        ...
        <div class="card p-16 text-center">
          <h2 class="text-4xl font-bold text-slate-900 mb-6 text-balance">Want to contribute?</h2>
          ...
          <div class="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            >
              <Icon name="github" size={6} class="mr-3" aria-label="GitHub" />
              Submit via GitHub
            </a>
            <button type="button" class="btn-secondary text-lg">
              <Icon name="email" size={6} class="mr-3" aria-label="Email" />
              Contact Us
            </button>
          </div>
        </div>
