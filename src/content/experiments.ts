...
export const experiments: readonly Experiment[] = [
  {
    ...
    whyItMatters:
      "Any Sharpe ratio, volatility-scaled holding, risk-parity weight, covariance matrix or Kelly fraction built on those two factors inherits a denominator error that will not shrink with more data.",
    whatWouldChangeIt: [
  ...
  },
  {
    ...
    keyNumbers: [
      ...
      { label: "Exposures surviving correction", value: "38 of 44", note: "Benjamini-Hochberg at 0.10" },
      ...
      { label: "Median minimum detectable alpha", value: "4.52", unit: "pp/yr" },
      {
        ...
        value: "−0.55",
        unit: "pp/yr",
        note: "VTI's own alpha under FF5+UMD, on error bars widened for clustered, uneven returns, t = −3.41. Read every alpha as a distance from this, not from zero",
      },
  ...
  },
  {
    ...
    number: "004",
    title: "What a trend holding adds to a portfolio that already exists",
    question:
      "Does a 15% time-series-momentum holding raise a passive portfolio's certainty-equivalent return against a risk-matched increase in cash?",
    kind: "exploratory",
    runState: "synthesised",
    status: "rejected",
    statusNote:
    ...
    keyNumbers: [
      ...
      { label: "Static + volatility replica", value: "+0.586", unit: "pp/yr", note: "43.7% of the holding" },
      { label: "The holding's margin over its own replica", value: "+0.756", unit: "pp/yr" },
    ...
    whatWouldChangeIt: [
      "A multi-asset attribution. The current one can only see the US equity market while the holding trades 58 instruments, so its 12.6% R-squared is a lower bound on what simple exposures explain.",
  ...
  },
  {
    ...
    whatWouldChangeIt: [
      "For RMW and CMA, roughly a further decade of months on data the rule had not seen (about 245, ending near 2035), since the detection threshold scales as one over the square root of the sample.",
  ...
  },
  {
    ...
    whyItMatters:
      "It reversed the earlier answer that momentum was the weakest factor here. The case against a momentum holding is now entirely about turnover, product shelf and a shared crash, not about the premium.",
    whatWouldChangeIt: [
  ...
  },
  {
    ...
    question: "What fraction of a long-short factor premium does a long-only lean actually deliver?",
    kind: "exploratory",
    runState: "synthesised",
    status: "rejected",
    statusNote:
      "What was rejected is not the capture fraction. It is the premise that there is one: five defensible benchmarks span 0.846, against a 0.30 threshold frozen in advance as the point at which a multiplier stops being a multiplier. A later correction went further: the fraction is itself an exposure to HML, so it may not multiply one.",
    verdict:
      "Against a size-neutral benchmark the capture is about 0.520, in the US, in developed ex-US, in emerging markets, and for momentum as well as value: a structural one half, because a long leg is one half of a symmetric spread. Against the capitalisation-weighted market the same lean reads 0.958, and the whole difference is a size premium wearing a value label. Regress that same spread on the factors and 94% of the 0.520 is simply its HML coefficient, 0.4891, so every chain of the form exposure × capture discounted one exposure twice and understated a long-only value lean by roughly a factor of two.",
    keyNumbers: [
      {
      ...
      {
        label: "HML exposure of the same size-neutral spread",
        value: "0.4891",
    ...
    whyItMatters:
      "It replaced the edge budget's assumed 0.40 with a measurement, showed the assumption was under-specified rather than wrong, and then removed the term from the budget altogether. A factor line is weight × (fund exposure − incumbent exposure) × premium − cost, and it carries no capture at all.",
    whatWouldChangeIt: [
      "A fund's delivered capture, which needs holdings rather than returns, and is Experiment 002's data rather than this one's. It now matters less, since a budget needs the exposure and not the ratio.",
      "Evidence that an exposure estimated on 36 to 72 months forecasts the next 36 to 72, which nothing here tests.",
    ],
  ...
  },
  {
    ...
    status: "exploratory",
    statusNote:
      "DBMF reached exploratory; CTA, FMF, KMLM and WTMF are rejected against the frozen 0.50 exposure bar. Decision 0002 caps the whole audit at exploratory.",
    verdict:
      "One product on the listed shelf delivers the exposure, and it is the one that sells replication. DBMF loads +0.671 with an interval clear of the bar, holds it across every split the window supports, and trailed a cost-free vendor index by less than it charges. The tax the funds distribute is two to three times their fee, and it is zero inside a shelter, but that is a fact about a pro-rata fund, not about the exposure: the same trend exposure through a return-stacked fund carries 0.32 pp/yr, of which 4.5 bp is incremental over the equity fund inside it. The way it is packaged decides more than the account does.",
    keyNumbers: [
      {
        label: "DBMF exposure to the AQR index",
        value: "+0.671",
    ...
    whatWouldChangeIt: [
      ...
      "A second product with an exposure at or above 0.50, which would remove DBMF's single-product risk.",
  ...
  },
  {
    ...
    keyNumbers: [
      ...
      {
        label: "Funds below the exposure bar on the US panel instead of their own region's",
        value: "16 of 25",
        note: "Against 5 on the correct regional panel. A published ex-US exposure without its panel named is not a number.",
      },
      {
  ...
  },
  {
    ...
    keyNumbers: [
      ...
      { label: "Exposures surviving correction", value: "96 of 109", note: "Benjamini-Hochberg at 0.10" },
      {
      ...
      },
      {
        ...
        value: "+0.537",
        interval: "[+0.43, +0.64]",
        note: "HML exposure over all 72 months, sign-adjusted for its mandate; it beat its in-sample cheap replication by 4.92 pp/yr and its alpha of +0.39 sits far inside a 3.64 detection threshold",
      },
    ],
    whyItMatters:
  ...
  },
  {
    ...
    number: "010",
    title: "What one added holding is worth inside a real portfolio",
    question:
    ...
    verdict:
      "Every holding except trend had been judged by a standalone chain, which sets the covariance term to zero by construction. Judged inside a portfolio instead, no holding survives, and the reason is a bound rather than a measurement: the diversification credit has a ceiling set by the base portfolio's own variance, and the ceiling is below the bar. The portfolio view rescues nothing the standalone chain dismissed, and against a 60/40 base it strengthens six of those dismissals instead. The bound and what it means for a trend holding are on the portfolio page.",
    keyNumbers: [
      { label: "Holdings tested", value: "10 in the Holm family, plus a modelled proxy and a control" },
      { label: "Holdings surviving Holm at 0.05", value: "0 of 10" },
      {
        label: "Holdings clearing the bar on the certainty equivalent, then failing it on growth",
        value: "6 of 7",
      },
      {
        label: "Sample",
    ...
    whyItMatters:
      "It bounds what the portfolio-level view can rescue, and a later test of gold shows the bound is reachable and still not enough. A zero-beta asset takes the entire credit and remains 0.10 pp/yr behind: the credit is 2.17 pp/yr per unit of weight against a standalone shortfall of 2.95, and both scale with the weight, so holding more cannot close the gap. The ceiling on the credit, and what it means for a trend or momentum holding, is set out on the portfolio page.",
    whatWouldChangeIt: [
      "A funding rule that pays for the holding by borrowing instead of by selling the base, and a control matched on borrowing across every arm. Gold was landed and tested on 2026-08-17: it lands exactly on the credit ceiling with a beta of zero and still fails, so the ceiling was never the binding term. Re-run with the holding paid for by borrowing rather than by selling something, its marginal contribution changes sign, from −0.40 to +0.18 pp/yr, a swing of 0.58 from the funding rule alone, against a bar of 0.30 — and it still clears neither the bar nor the smallest effect the test could have found.",
...
export const ledgerSummary = {
  entries: 134,
  runs: 47,
  distinctSpecifications: 27,
  experimentFamilies: 23,
  runsRecordingResultsViewed: 39,
  ...
  terminalOutcomes: [
    ...
    },
    {
      ...
      runs: 21,
      which:
        "Experiments 002, 008 and 009, three executions each; Experiments 005 and 006; Experiment 013, two executions; Experiments 014 and 015; Experiment 016 and its four follow-ons, 016b, 016c, 016d and 016e; Experiment 017",
    },
  ],
  noTerminalStatus: {
    runs: 8,
  ...
  note: "Twenty-seven, not forty-seven, is the number a deflated-Sharpe trial count starts from: repeated executions of one specification are not independent hypotheses. Twenty-seven is itself an upper bound, because Experiment 010b re-judges data Experiment 010 had already spent, because Experiments 013, 014 and 015 re-run an earlier falsifier on data it had already spent rather than asking a new question, because Experiments 016b, 016c, 016d and 016e re-score 016's arms on the identical panel — 016b and 016e adding arms chosen after seeing its results, and 016c and 016d changing no arm at all and only sweeping an input — and because Experiment 017 reads no market data at all, so it tests no hypothesis about markets and belongs in no trial count.",
  source: {
