...
- [Where outperformance can come from](research/expected-edge-decomposition.md) —
  ...
  cheap index). Also settles why the investor behaviour gap is measured against a
  different benchmark and may never be added to the contractual budget.
...
- [0007 — The application may render research findings](decisions/0007-application-may-render-research.md)
  — why the ban on showing research numbers in the client was lifted, and the four
  constraints that replace it: one canonical content layer, status and date travelling
  with every number, benchmarks that never aggregate, and client arithmetic tested
  against fixtures generated by the research workspace.
- [0008 — Geometric growth decides; the certainty equivalent reports beside it](decisions/0008-growth-decides-crra-reports.md)
  — the CRRA metric pays a sleeve for de-risking, measured at +0.809 pp/yr on a control
  that supplies nothing; which experiments move as a result, and the risk-matched
  comparator a certainty-equivalent primary now requires.
