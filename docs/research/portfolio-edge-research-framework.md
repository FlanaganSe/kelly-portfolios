...
| **Size (SMB)** | **not signable, on any panel** | Pooled over three regions **+0.33 pp/yr** `[−1.32, +2.06]` against a **2.47** detection floor — the sharpest size instrument here. Developed ex-US +0.49 against 2.83, emerging −0.05, US +0.29 post-Banz; the US quintile spread reads +1.9 `[−1.9, +6.0]` against 4.73. **The ex-US legs were measured rather than transferred**, because HML is three times larger abroad ([factor persistence, §Size](factor-persistence.md#size-on-the-three-panels--a-study-not-an-experiment)) | A premium visible in some window this repository can reach. **Until then no chain may price an SMB loading**, which is what puts a large-value fund ahead of a small-value one on both shelves |
