...
function proseOf(text, ext) {
  ...
  if (ext === ".astro" || ext === ".md" || ext === ".mdx") s = s.replace(/^---\r?\n[\s\S]*?\r?\n---\r?\n/, "");
  s = s.replace(/```[\s\S]*?```/g, "");
  s = s.replace(/`[^`\n]*`/g, "");
  ...
  if (ext === ".ts" || ext === ".tsx" || ext === ".astro") {
    ...
    s = s.replace(
      /\b(?:href|src|id|key|slug|path|docPath|anchor|ticker|factor|route|file|category)\s*:\s*(["'])[^"'\n]*\1/g,
      ""
    );
...
function density(prose) {
  ...
  if (puffed > 0 && bland / puffed < 3)
    warn(`"serves as"/"represents" family outweighs plain "is" (${bland}:${puffed}, want >= 3:1)`);

  const participles = (prose.match(/,\s\w+ing\b/g) ?? []).length;
  if (per(participles, 1000) > 3)
    warn(`comma-offset "-ing" tails ${per(participles, 1000).toFixed(1)}/1k words (want <= 3)`);
  ...
  const naked = sentences(prose).filter(
    (s) =>
      ...
      !/\b(against|compared with|compared to|versus|vs\.?|relative to|better than|than|benchmark|index|control)\b/i.test(
        s
      )
  );
