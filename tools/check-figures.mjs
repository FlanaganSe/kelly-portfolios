...
for (const file of files) {
  ...
  const records = split
    ...
    : Array.isArray(doc)
      ? doc
      : (doc?.figures ?? Object.values(doc ?? {}));

  for (const r of records) {
    if (!r?.id) {
      problems.push(`${page}: a record has no id`);
      continue;
    }
    ...
    problems.push(
      `COLLISION ${id}: declared in ${uses.map((u) => u.page).join(", ")} with different facts:\n` +
        uses
          .map(
            (u) =>
              `      ${u.page}: value=${JSON.stringify(u.r.value)} unit=${JSON.stringify(u.r.unit ?? null)} interval=${JSON.stringify(u.r.interval ?? null)} status=${u.r.status}`
          )
          .join("\n")
    );
  }
